...
function(declare, lang, html, array, baseFx, on, topic, _TemplatedMixin, template, BaseWidgetPanel, IFramePane,
  LoadingIndicator, require) {

  return declare([BaseWidgetPanel, _TemplatedMixin], {
    baseClass: 'jimu-widget-panel jimu-dockable-panel',
    ...
    //  If the widgets count is larger then this number, there will be a navigation bar displayed to scroll the widget.
    displayNumber: 3,
    ...
    //resizeMap: Boolean
    //  if true, when resize the panel, the panel will resize map to make sure the panel will not override the map.
    //  Please note that when the panel resize the map, make sure that there is no other element between the panel and the map
    resizeMap: true,
    ...
    _setFrameSize: function(){
      ...
      var dim = {}, box = html.getContentBox(this.containerNode);
    ...
    loadDisplayWidgets: function(){
      ...
      for(var i = this.firstIndex; i < endIndex; i++){
        ...
        this.widgetManager.loadWidget(widgetConfig).then(lang.hitch(this, onWidgetLoaded, frame, loading));
      }
      this._setFrameFloat();
      this._setFrameSize();
    },
    ...
    _setBarPosition: function(){
      ...
      var pos = this.windowState === 'normal'? (0 - this._barSize) + 'px': (0 - this._barSize - 2) + 'px';
      if(this.region === 'left'){
        html.setStyle(this.barNode, {
          top: '10px',
          right: pos
